...
provides:
  ...
  - "Landing page rendering AI Finance Coach heading + Get started Button"
...
**Runnable Next.js 16 scaffold with strict TS, Vitest 4 sanity-tested, and shadcn/ui New York primitives (Button, Card, Input, Label) rendering the AI Finance Coach landing page.**
...
- **Landing page** at `app/page.tsx` renders an `h1` with "AI Finance Coach", a muted-foreground subtitle, and a styled shadcn `<Button>` reading "Get started". Verified via background `npm run dev` + `curl http://localhost:3000` → HTTP 200, body contained both required strings.
...
- Dev server returned HTTP 200 with body containing "AI Finance Coach" and "Get started"; subsequently terminated.
